...
| W6.1 | Partial; sender partial-write correctness | A pinned payload write preserves progress across supersession/producer closure; write/FIN errors fail, and a frame guard resets on cancellation/error under one 30-second deadline. Failing-before exact-byte regressions and real iroh/noq RESET coverage pass. Whole serving-session ownership, reference-aware queue dropping, wire session IDs and native presentation/network gates remain open; see [contract](desktop-frame-delivery.md). |
